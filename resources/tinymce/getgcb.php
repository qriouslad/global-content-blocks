        ...
        <script type="text/javascript">
            function do_s() {
				...
				if(opt.value != "0")
				{
					...
					if(!actual) {
						var img = opt.id;
						var html = "<img src='<?php echo WP_PLUGIN_URL; ?>/global-content-blocks/resources/i/"+img+"' class='gcbitem mceItem' title='contentblock id=" + opt.value + " img="+img+"' />";
					}
					else
					{
						var html = "";
						continue_send = false;
						jQuery.post(
						"<?php echo WP_PLUGIN_URL; ?>/global-content-blocks/resources/tinymce/gcbvalue.php",
						{id:opt.value},
						function(data){
			...
			jQuery(document).ready(function(ex){     				
				...
				<?php if(current_user_can('publish_pages'))://allow only editors and above to add blocks ?>
				...
				jQuery("#gcb_frame_add_do").click(function(){
					...
					jQuery.post(
					"<?php echo WP_PLUGIN_URL; ?>/global-content-blocks/resources/tinymce/gcb_ajax_add.php",
    ...
    <body>		
		<div id="insert_wrapper">
			...
			<select id="gcb_sel" name="gcb_sel" style="width:300px;">
				...
				<option id="<?php echo $available_types[$entry["type"]]["img"]?>" value="<?php echo $entry["id"];?>"><?php echo $entry["name"]; ?></option>
				<?php endforeach; ?>
			</select>
			<br /><br />
